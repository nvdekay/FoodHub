...
* Khách hàng cần đăng ký/đăng nhập tài khoản trước khi đặt món (để có thể sửa/xoá đơn của chính mình và theo dõi lịch sử).

* Mỗi đơn hàng gắn với một bàn trong cửa hàng (mô hình đặt món tại chỗ).
...
| Mã | Chức năng | Mô tả & Quy tắc nghiệp vụ |
| :---- | :---- | :---- |
| FR-ORD-01 | Thêm món vào đơn | Khách chọn món, số lượng, tuỳ chọn (size/topping), ghi chú. Hệ thống tính itemTotal \= (giá gốc \+ tổng phụ phí tuỳ chọn) × số lượng. |
| FR-ORD-02 | Tạo đơn đặt món | Khách xác nhận giỏ hàng → tạo order với trạng thái pending. Hệ thống sinh orderCode duy nhất, tính subtotal và totalAmount, snapshot thông tin từng món. |
...
| FR-STAFF-02 | Xác minh & xác nhận đơn | Nhân viên kiểm tra đơn pending và xác nhận → chuyển trạng thái confirmed, ghi confirmedBy và confirmedAt. |
| FR-STAFF-03 | Cập nhật trạng thái chế biến | Chuyển trạng thái theo luồng: confirmed → preparing → ready → completed. Mỗi lần chuyển ghi vào statusHistory. |
| FR-STAFF-04 | Huỷ đơn | Nhân viên có thể huỷ đơn (cancelled) kèm lý do (cancelReason) ở các trạng thái phù hợp. |
| FR-STAFF-05 | Cập nhật thanh toán | Đánh dấu paymentStatus (unpaid → paid) và phương thức (cash/card/ewallet). |
...
| FR-TBL-02 | Theo dõi trạng thái bàn | Trạng thái bàn: available / occupied / reserved. Khi có đơn đang phục vụ, bàn ở trạng thái occupied. |

## **4.6. Module Thống kê (Dashboard)**

| Mã | Chức năng | Mô tả & Quy tắc nghiệp vụ |
| :---- | :---- | :---- |
| FR-DASH-01 | Thống kê tổng quan | Số đơn theo trạng thái, doanh thu trong ngày/tuần, số đơn hôm nay. |
| FR-DASH-02 | Món bán chạy | Top món bán chạy dựa trên tổng số lượng trong các đơn đã hoàn thành. |
...
| 9 | **discountAmount** | Number | Không | Số tiền giảm giá (nếu có). Mặc định 0\. |
...
| 12 | **paymentStatus** | String (enum) | **Có** | 'unpaid' | 'paid' | 'refunded'. Mặc định 'unpaid'. |
...
| preparing | Đang chế biến. | NV: chuyển sang ready. |
| ready | Đã làm xong, sẵn sàng phục vụ. | NV: chuyển sang completed. |
| completed | Đã giao cho khách, hoàn tất. | Trạng thái kết thúc (cộng soldCount). |
| cancelled | Đơn bị huỷ. | Trạng thái kết thúc (kèm cancelReason). |
